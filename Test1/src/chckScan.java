...
public class chckScan {
    ...
    {
        square king = board.findKing(Piece.isWhite("white"));
        assert king != null;
    ...
    private boolean hitByRook(int col, int row, square king, int kingCol, int kingRow, int colVal, int rowVal)
    {
        for (int i = 1; i< 8;i++)
        {
            ...
            if(piece != null && piece != square.oldPiece){
                ...
                if (!Piece.isSameTeam(piece, king.piece ) && (piece.icon == new ImageIcon(getClass().getClassLoader().getResource("resources/rock_white-removebg-preview (2).png"))
